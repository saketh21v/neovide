...
fn create_platform_shell_command(command: &str, args: &[&str]) -> Option<StdCommand> {
    if cfg!(target_os = "windows") && SETTINGS.get::<CmdLineSettings>().wsl {
        ...
        let mut n_command = command;

        if shell.ends_with("nu") && n_command == "which" {
            n_command = "^which"
        }

        let mut shell_args = "-lc";
        if shell.ends_with("nu") {
            shell_args = "-c"
        }

        result.args(&[shell_args]);
        result.arg(format!("{} {}", n_command, args.join(" ")));
        Some(result)
    } else {
        None
    }
...
fn build_nvim_cmd_with_args(bin: &str) -> TokioCommand {
    ...
    if cfg!(target_os = "windows") && SETTINGS.get::<CmdLineSettings>().wsl {
        ...
        cmd.args(&[shell_args, &format!("{} {}", bin, args_str)]);
        cmd
    } else {
